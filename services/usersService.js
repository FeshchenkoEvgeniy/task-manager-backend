...
const loginUser = async (id, token) => {
  try {
    const user = await User.findByIdAndUpdate(id, { token }, { select: "-password -token -createdAt -updatedAt" });
    return user;
  } catch (error) {
    return error;
  }
...
const changeColorTheme = async (id, theme) => {
  try {
    const user = await User.findByIdAndUpdate(id, { theme: theme }, { new: true, select: "theme -_id" });
    return user;
  } catch (error) {
    return error;
  }
